(function(angular){
	...
	.controller('FileTypeCtrl',[
		...
		'FileUploader',
		'FileType',
		function($scope,Order,FileUploader,FileType){
			var vm = $scope.vm = {};
...
//	  			{label: '排列',name: 'sort',type: 'string'},
			...
			vm.getList = function(params){
				FileType.getList(params).then(function(answer){vm.items = answer});
			};
			vm.getById = function(id){
				FileType.getById(id).then(function(answer){vm.Item = answer});
			};

			vm.create = function(obj){
				FileType.create(obj).then(vm.resolveCreate)
				$('#createModal').modal('hide');
			};
			vm.update = function(obj){
				FileType.update(obj).then(vm.resolveUpdate);
				$('#updateModal').modal('hide');
			};

			vm.remove = function(id){
				FileType.remove(id).then(vm.resolveRemove);
