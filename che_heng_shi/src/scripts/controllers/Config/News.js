(function(angular){
	...
	.controller('NewsCtrl',[
		...
		'FileUploader',
		'News',
		function($scope,$filter,FileUploader,News){
			var vm = $scope.vm = {};
			...
			vm.getList = function(params){
				News.getList(params).then(function(answer){vm.items = answer});
			};
			vm.getById = function(id){
				News.getById(id).then(function(answer){vm.Item = answer});
			};
			
			vm.create = function(obj){
				obj.text = obj.text?$filter('unicodeToString')(obj.text):''
				News.create(obj).then(vm.resolveCreate)
				$('#createModal').modal('hide');
			};
			vm.update = function(obj){
				obj.text = obj.text?$filter('unicodeToString')(obj.text):''
				News.update(obj).then(vm.resolveUpdate);
				$('#updateModal').modal('hide');
			};
			
			vm.remove = function(id){
				News.remove(id).then(vm.resolveRemove);
