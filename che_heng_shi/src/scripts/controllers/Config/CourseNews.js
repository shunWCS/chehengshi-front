(function(angular){
    angular.module('module.controller')
        .controller('CourseNewsCtrl',[
            ...
            'FileUploader',
            'CourseNews',
            function($scope,$filter,FileUploader,CourseNews){
                var vm = $scope.vm = {};
                ...
                vm.getList = function(params){
                    CourseNews.getList(params).then(function(answer){vm.items = answer});
                };
                vm.getById = function(id){
                    CourseNews.getById(id).then(function(answer){vm.Item = answer});
                };

                vm.create = function(obj){
                    obj.text = obj.text?$filter('unicodeToString')(obj.text):''
                    CourseNews.create(obj).then(vm.resolveCreate)
                    $('#createModal').modal('hide');
                };
                vm.update = function(obj){
                    obj.text = obj.text?$filter('unicodeToString')(obj.text):''
                    CourseNews.update(obj).then(vm.resolveUpdate);
                    $('#updateModal').modal('hide');
                };

                vm.remove = function(id){
                    CourseNews.remove(id).then(vm.resolveRemove);
