(function(angular){
    angular.module('module.controller')
        .controller('PassVideoCtrl',[
            ...
            'FileUploader',
            'PassVideo',
            function($scope,$filter,$rootScope,FileUploader,PassVideo){
                var vm = $scope.vm = {};
                ...
                vm.getList = function(params){
                    PassVideo.getList(params).then(function(answer){vm.items = answer});
                };
                vm.getById = function(id){
                    PassVideo.getById(id).then(function(answer){vm.Item = answer});
                };

                vm.create = function(obj){
                    obj.text = obj.text?$filter('unicodeToString')(obj.text):''
                    PassVideo.create(obj).then(vm.resolveCreate)
                    $('#createModal').modal('hide');
                };
                vm.update = function(obj){
                    obj.text = obj.text?$filter('unicodeToString')(obj.text):''
                    PassVideo.update(obj).then(vm.resolveUpdate);
                    $('#updateModal').modal('hide');
                };

                vm.remove = function(id){
                    PassVideo.remove(id).then(vm.resolveRemove);
