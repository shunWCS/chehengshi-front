(function(angular){
    angular.module('module.controller')
        .controller('PosterTypeCtrl',[
            ...
            'FileUploader',
            'PosterType',
            function($scope,Order,FileUploader,PosterType){
                var vm = $scope.vm = {};
...
//	  			{label: 'ID',name: 'id',type: 'string'},
                    ...
                    {label: '中文类型',name: 'typeName',type: 'string'},
                    {label: '英文类型',name: 'typeValue',type: 'string'},
                ...
                vm.getList = function(params){
                    PosterType.getList(params).then(function(answer){vm.items = answer});
                };
                vm.getById = function(id){
                    PosterType.getById(id).then(function(answer){vm.Item = answer});
                };

                vm.create = function(obj){
                    PosterType.create(obj).then(vm.resolveCreate)
                    $('#createModal').modal('hide');
                };
                vm.update = function(obj){
                    PosterType.update(obj).then(vm.resolveUpdate);
                    $('#updateModal').modal('hide');
                };

                vm.remove = function(id){
                    PosterType.remove(id).then(vm.resolveRemove);
