(function(angular){
    angular.module('module.controller')
        .controller('CourseTypeCtrl',[
            ...
            'FileUploader',
            'CourseType',
            function($scope,Order,FileUploader,CourseType){
                var vm = $scope.vm = {};
...
//	  			{label: '排列',name: 'sort',type: 'string'},
                ...
                vm.getList = function(params){
                    CourseType.getList(params).then(function(answer){vm.items = answer});
                };
                vm.getById = function(id){
                    CourseType.getById(id).then(function(answer){vm.Item = answer});
                };

                vm.create = function(obj){
                    CourseType.create(obj).then(vm.resolveCreate)
                    $('#createModal').modal('hide');
                };
                vm.update = function(obj){
                    CourseType.update(obj).then(vm.resolveUpdate);
                    $('#updateModal').modal('hide');
                };

                vm.remove = function(id){
                    CourseType.remove(id).then(vm.resolveRemove);
