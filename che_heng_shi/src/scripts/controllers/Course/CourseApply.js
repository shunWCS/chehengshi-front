(function(angular){
    angular.module('module.controller')
        .controller('CourseApplyCtrl',[
            ...
            'FileUploader',
            'CourseApply',
            function($scope,$filter,$rootScope,FileUploader,CourseApply){
                var vm = $scope.vm = {};
                ...
                vm.columns = [
                    ...
                    {label: '姓名',name: 'applyName',type: 'string'},

                    {label: '课程名称',name: 'courseName',type: 'string'},
                    {label: '联系电话',name: 'phone',type: 'string'},
                    {label: '报名日期',name: 'createTime',type: 'string'},
                    {label: '备注信息',name: 'remark',type: 'string'},
                ...
                vm.getList = function(params){
                    CourseApply.getList(params).then(function(answer){vm.items = answer});
                };
                vm.getById = function(id){
                    CourseApply.getById(id).then(function(answer){vm.Item = answer});
                };

                vm.create = function(obj){
                    obj.text = obj.text?$filter('unicodeToString')(obj.text):''
                    CourseApply.create(obj).then(vm.resolveCreate)
                    $('#createModal').modal('hide');
                };
                vm.update = function(obj){
                    obj.text = obj.text?$filter('unicodeToString')(obj.text):''
                    CourseApply.update(obj).then(vm.resolveUpdate);
                    $('#updateModal').modal('hide');
                };

                vm.remove = function(id){
                    CourseApply.remove(id).then(vm.resolveRemove);
